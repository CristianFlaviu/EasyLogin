...
import { TenantRoleDialogComponent } from '../role-dialog/tenant-role-dialog.component';
import { ConfirmDialogComponent } from '../../../shared/components/confirm-dialog/confirm-dialog.component';

@Component({
  selector: 'app-tenant-role-list',
  standalone: true,
  imports: [
    DatePipe,
  ...
  templateUrl: './tenant-role-list.component.html',
  styleUrl: './tenant-role-list.component.scss',
})
export class TenantRoleListComponent implements OnInit {
  ...
  openCreateDialog(): void {
    const ref = this.dialog.open(TenantRoleDialogComponent, { width: '420px' });
    ref.afterClosed().subscribe(result => { if (result) this.loadRoles(); });
  }

  deleteRole(role: TenantRoleItem): void {
    ...
    ref.afterClosed().subscribe(confirmed => {
      if (!confirmed) return;
      this.tenantAdmin.deleteRole(role.id).subscribe({
        next: () => {
